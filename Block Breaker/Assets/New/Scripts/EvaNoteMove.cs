...
public class EvaNoteMove : MonoBehaviour
{
    // Define keys
    ...
    void Start()
    {
        float bpm = EvangelionGameManager.instance.bpm;
        beatTempo = bpm / 60f;
    ...
    private void ClickAttempt()
    {
        if (Input.GetKeyDown(keyToPress1) || Input.GetKeyDown(keyToPress2) || Input.GetKeyDown(KeyCode.Mouse0) || Input.GetKeyDown(KeyCode.Mouse1))
        {
            if (canBePressed)
            {
                ...
                EvangelionGameManager.instance.NoteHit();
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.tag == "Activator")
        {
            ...
            EvangelionGameManager.instance.NoteMissed();
        }
    }
